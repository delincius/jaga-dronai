...
from drone_search import generate_lawnmower_path_ned

area = [
        (0.0, 0.0),        # Apatinis kairys kampas
        (100.0, 0.0),      # Apatinis dešinys kampas
        (100.0, 100.0),    # Viršutinis dešinys kampas
        (0.0, 100.0)       # Viršutinis kairys kampas
    ]
    
    # Generuojame "lawnmower" tipo trajektoriją
waypoints = generate_lawnmower_path_ned(area, step=10.0)

# Išvedame suformuotus taškus
print("Sugeneruoti taškai:")
for wp in waypoints:
    print(f"({wp.north_m}, {wp.east_m}, {wp.down_m})")
...
async def fly_area(drone):
    """Skraidinam droną teritorijoje."""
    ...
    print(f"Dronas -  {drone} - užbaigė skrydį teritorijoje!")
...
async def control_drones():
    ...
    # await asyncio.gather(
    #     fly_drone_forward(drones[0], 0.0, 0.0, -5.0, 0.0),

    # )
    
    # await asyncio.sleep(7)  # šiek tiek laiko stabilizuotis
